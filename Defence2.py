...
def Tackels_Team2_and_Defence_Locations_Players(chart_type, df, selected_player):
    if chart_type == 'Total':
        ...
        if not filtered_df.empty:
            ...
            st.warning("# No Successful Tackle took place.")
...
def Team2_defender_Video_data(df, unique_r_nos, tackle_points_values):
...
##-------------------------------------------------defender out by attacks -------------------------------------------#

def Team2_Defender_out(df, selected_player):
    filtered_df = df[(df['RD'] == 'R') & ((df['Defence player No.'] == selected_player) | (df['2nd Player Out'] == selected_player))]
    ...
    if not filtered_raider_attacks.empty:
        ...
        fig.update_layout(
            ...
            title='Successful Attack Locations on Defender',
            showlegend=True,
        )
