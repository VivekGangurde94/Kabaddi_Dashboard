...
def Team1_bonus_points_distribution_Raider(df, r_no):
    # Filter DataFrame for bonus point attempts and bonus points
    bonus_points_df = df[(df['RD'] == 'R') & (df['R No.'] == r_no)][['Bonus Point attempt', 'Bonus Points']]
...
def Team1_raider_escape_Raider(df,r_no):
    filtered_df = df[(df['RD'] == 'R') & (df['R No.'] == r_no)]  # Filter rows where RD column is 'R'
...
def Team1_Attack_and_Attack_Locations_Raider(chart_type, df, r_no):
    if chart_type == 'Total':
        filtered_attacks = df[(df['RD'] == 'R') & (df['R No.'] == r_no)][['Raider Attack', 'Second Attack Raider']].stack()
...
### ----------------------------------------------attack location can sorting attack location -------------------------------------------------------------------------------------##
# def Team1_Attack_and_Attack_Locations_Raider(chart_type, df, r_no):
#     if chart_type == 'Total':
#         filtered_attacks = df[(df['RD'] == 'R') & (df['R No.'] == r_no)][['Raider Attack', 'Second Attack Raider']].stack()
#         filtered_attacks = filtered_attacks[filtered_attacks != 'None']
...
#             # Loop through each attack location and add it to the figure with a different color and label
#             for i in range(len(all_start_coordinates)):
#                 start_x = all_start_coordinates.iloc[i, 0]
#                 start_y = all_start_coordinates.iloc[i, 1]
#                 end_x = all_end_coordinates.iloc[i, 0]
#                 end_y = all_end_coordinates.iloc[i, 1]
#                 attack_name = all_raider_attack_names.iloc[i]
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------##
        ...
        filtered_attacks = df[(df['RD'] == 'R') & (df['R No.'] == r_no)]
...
##-------------------------------------Touch_Points_Score at player present at without animation code -------------------------------------------------###

# def Touch_Points_at_Player_present_Raider(df,r_no):
#  # Filter the DataFrame based on the condition RD == 'R' and the raider number
#     filtered_df = df[(df['RD'] == 'R') & (df['R No.'] == r_no)]
...
##-------------------------------------Touch_Points_Score at player present at with animation code -------------------------------------------------###
...
def Touch_Points_at_Player_present_Raider(df, r_no):
    ...
    if not grouped_df.empty:
        # Create donut chart trace
        labels = grouped_df['Total Players at Defence'].astype(str) + " Defender"
        values = grouped_df['Touch Points']
        trace = go.Pie(labels=labels, values=values, name='Touch Points', hole=0.5)

        # Create frames for animation
        frames = [go.Frame(data=[go.Pie(labels=labels[:i+1], values=values[:i+1], name='Touch Points', hole=0.5)]) for i in range(len(grouped_df))]

        # Create layout
        layout = go.Layout(title='Points Scored with Number of Defenders Present', updatemenus=[dict(type='buttons', showactive=False, buttons=[dict(label='Play', method='animate', args=[None, {'frame': {'duration': 500, 'redraw': True}, 'fromcurrent': True}]), dict(label='Pause', method='animate', args=[[None], {'frame': {'duration': 0, 'redraw': True}, 'mode': 'immediate', 'transition': {'duration': 0}}])])])
        
        # Create figure
        fig = go.Figure(data=[trace], frames=frames, layout=layout)
        fig.update_traces(textinfo='label+value')

        # Display the animated donut chart using plotly_chart
        st.plotly_chart(fig)
    else:
        st.warning("No touch points found for the selected player.")
...
##------------------------------------do-or-Die Raids--------------------------------------------------------------------------------------------##
def do_or_die_Team1_Raider(df,r_no):
...
def Team1_Raider_tackled(df,r_no):
    # Filter DataFrame for rows where RD column is R and Tackle Point is 1
    raider_tackled_df = df[(df['RD'] == 'R') & (df['R No.'] == r_no) & (df['Tackle Points'] == 1)]
    
    if raider_tackled_df.empty:
        st.warning("# No raider were tackled.")
        return
...
def Team1_Scorecard_players(df, selected_player):
    ...
    if selected_player:
        team1_combined_data = team1_combined_data.loc[selected_player]

    if not team1_combined_data.empty:
        team1_combined_data.rename_axis('Jersey No.', inplace=True)

    return team1_combined_data
